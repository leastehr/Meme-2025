...
with st.sidebar:
    ...
    with st.expander("ℹ️ How to use", expanded=False):
        ...
        <div style="color: white;">
            1. Choose a template or keep pressing Select template five times.<br>
    ...
    if templates and st.button("Select template 🔄", key="rand_btn") and st.session_state["skips"] < 5:
        ...
        for k in list(st.session_state.keys()):
            if k.startswith("txt") or k.startswith("pos_select") or k.startswith("x") or k.startswith("y") or k.startswith("color"):
    ...
    for i in range(num_caps):
        ...
        selected = st.selectbox("Position", POSITION_OPTIONS, index=POSITION_OPTIONS.index("Top"), key=f"pos_select{i}")
        color = st.color_picker("Text color", "#FFFFFF", key=f"color{i}")
        ...
        if selected == "Top":
            ...
            if f"x{i}" not in st.session_state:
                st.session_state[f"x{i}"] = 50
            if f"y{i}" not in st.session_state:
